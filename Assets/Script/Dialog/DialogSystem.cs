...
{
	[SerializeField]
	protected Speaker[]		speakers;					// 대화에 참여하는 캐릭터들의 UI 배열
	[SerializeField]
	protected DialogData[]	dialogs;					// 현재 분기의 대사 목록 배열
	[SerializeField]
	protected	bool			isAutoStart = true;         // 자동 시작 여부
	protected bool			isFirst = true;             // 최초 1회만 호출하기 위한 변수
	protected int				currentDialogIndex = -1;    // 현재 대사 순번
	protected int				currentSpeakerIndex = 0;    // 현재 말을 하는 화자(Speaker)의 speakers 배열 순번
	protected float typingSpeed = 0.1f;         // 텍스트 타이핑 효과의 재생 속도
	protected bool isTypingEffect = false;		// 텍스트 타이핑 효과를 재생중인지
